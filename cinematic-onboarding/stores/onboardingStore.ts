...
import { SceneNumber, SceneStatus, QualityTier } from "@/types";
...
interface OnboardingState {
  ...
  /**
   * Currently focused 3D object ID via keyboard (null if none)
   */
...
const getSceneFromProgress = (progress: number): SceneNumber => {
  ...
  if (progress >= 1.00) return 7; // Initially show Scene 7 (Constellation of Reasoning)
  return 1;
};
...
const detectQualityTier = (): QualityTier => {
  if (typeof window === 'undefined') return QualityTier.MEDIUM;
  
  // Check for GPU tier via WebGL
  const canvas = document.createElement('canvas');
  const gl = canvas.getContext('webgl') || canvas.getContext('experimental-webgl');
  
  if (!gl) return QualityTier.LOW;
  ...
  if (cores >= 8 && memory >= 8 && width >= 1920) return QualityTier.ULTRA;
  if (cores >= 4 && memory >= 4 && width >= 1280) return QualityTier.HIGH;
  if (cores >= 2 && memory >= 2) return QualityTier.MEDIUM;
...
export const useOnboardingStore = create<OnboardingState>()(
  persist(
    (set, get) => ({
      ...
      sceneStatuses: {
        1: SceneStatus.UNLOADED,
        2: SceneStatus.UNLOADED,
        3: SceneStatus.UNLOADED,
        4: SceneStatus.UNLOADED,
        5: SceneStatus.UNLOADED,
        6: SceneStatus.UNLOADED,
        7: SceneStatus.UNLOADED,
        8: SceneStatus.UNLOADED,
      },
      ...
      setScrollProgress: (progress) => {
        ...
        let newScene = getSceneFromProgress(clampedProgress);
        
        // Prevent jumping back to Scene 7 if we've already transitioned to Scene 8 at progress 1.00
        if (clampedProgress >= 1.00 && current === 8) {
          newScene = 8;
        }
      ...
      isSceneReady: (scene) => {
        ...
        return status === SceneStatus.READY || status === SceneStatus.ACTIVE;
      },
...
export const useShouldUsePostProcessing = (): boolean => {
  return useOnboardingStore((state) => 
    state.qualityTier !== QualityTier.LOW && !state.reducedMotion
  );
};
