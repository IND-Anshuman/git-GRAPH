    ...
    def discover_capabilities(self, uow: IUnitOfWork, repository_id: uuid.UUID) -> List[CapabilityCandidate]:
        ...
        if concepts:
            ...
            for c in concepts:
                ...
                if hasattr(uow, "concept_evidence"):
                    evidences = uow.concept_evidence.get_by_concept_node(c.id) if hasattr(uow.concept_evidence, "get_by_concept_node") else []
                    for ev in evidences:
                        features.add(str(ev.target_id))
                        if ev.metadata_ and "file_path" in ev.metadata_:
                            features.add(ev.metadata_["file_path"])
